  ...
  <div class="chart-container q-my-lg">
    ...
    <div v-show="!request_pending">
      <div class="flex justify-between">
        <div class="indicator-chart-title">{{ getRenderedCustomGeometryData?.name }} Soil Crust Factor </div>
        <!---------------------------------------  chart actions ----------------------------------------->
        <div class="chart-actions flex">
          ...
          <!-- link to information -->
          <div flat class="q-ma-xs cursor-pointer">
            <a href="https://misland.readthedocs.io/en/latest/Service/Calculate_erosion.html#calculate-wind-erosion" target="_blank"
...
export default {
  ...
  methods: {
    ...
    async fetchYears() {
      this.years = await fetchComputationYears("ILSWE");
    ...
    async submit() {
      try {
        ...
        const results = await requestAnalysis({ uri, payload, caller: 'ilswe', indicator: this.indicator, hide_loading_progress: true });// request analysis
