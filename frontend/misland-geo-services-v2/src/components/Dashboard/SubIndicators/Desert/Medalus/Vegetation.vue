  ...
  <div class="chart-container q-my-lg">
    ...
    <div v-show="!request_pending">
      <div class="flex justify-between">
        <div class="indicator-chart-title">{{ getRenderedCustomGeometryData?.name }} Vegetation quality index </div>
        <!---------------------------------------  chart actions ----------------------------------------->
          ...
          <div class="q-ma-xs cursor-pointer relative-position">
            ...
            <div class="dashboard-form-menu" v-show="show_selection_form">
              ...
              <div class="dashboard-product-selections">
                ...
                <div class="q-my-sm">
                  <div class="label q-py-sm">SELECT QUALITY INDEX</div>
...
export default {
  ...
  methods: {
    ...
    async fetchYears() {
      this.years = await fetchComputationYears("Climate Quality Index");
    ...
    async submit() {
      try {
        ...
        const uri = `/api/vegetationquality/`; // build the request url
        this.request_pending = true
        const results = await requestAnalysis({ uri, payload, caller: 'vegetationquality', indicator: this.indicator, hide_loading_progress: true });// request analysis
