  ...
  <div class="chart-container q-my-lg">
    ...
    <div v-show="!request_pending">
      <div class="flex justify-between">
        <div class="indicator-chart-title">{{ getRenderedCustomGeometryData?.name }} Management quality index </div>
        <!---------------------------------------  chart actions ----------------------------------------->
  ...
  methods: {
    ...
    async submit() {
      try {
        ...
        const uri = `/api/managementquality/`; // build the request url
        this.request_pending = true
        const results = await requestAnalysis({ uri, payload, caller: 'managementquality', indicator: this.indicator, hide_loading_progress: true });// request analysis
