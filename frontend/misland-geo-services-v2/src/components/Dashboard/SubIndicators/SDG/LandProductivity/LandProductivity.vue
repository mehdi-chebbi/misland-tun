  ...
  <div class="chart-container q-my-lg">
    ...
    <div v-show="!request_pending">
      <div class="flex justify-between">
        <div class="indicator-chart-title">{{ getRenderedCustomGeometryData?.name }} Land productivity</div>
        <!---------------------------------------  chart actions ----------------------------------------->
...
export default {
  ...
  data() {
    return {
      ...
      veg_index: "NDVI",
      end_year: 2019,
      indicator: { label: "Land productivity", value: "productivity", colors },
  ...
  methods: {
    ...
    async fetchYears() {
      this.years = await fetchComputationYears("Productivity")
    },
    ...
    async submit() {
      try {
        ...
        const uri = `/api/productivity/`; // build the request url
        this.request_pending = true
        const results = await requestAnalysis({ uri, payload, caller: 'productivity', indicator: this.indicator, hide_loading_progress: true });// request analysis
    ...
    processStatsSummary() {
      // summary for state
      this.summary_html = this.$t("summaries.land_productivity", [
