  ...
  <div class="chart-container q-my-lg">
    ...
    <div v-show="!request_pending">
      <div class="flex justify-between">
        <div class="indicator-chart-title">{{ getRenderedCustomGeometryData?.name }} performance </div>
        <!---------------------------------------  chart actions ----------------------------------------->
        <div class="chart-actions flex">
          ...
          <!-- link to information -->
          <div flat class="q-ma-xs cursor-pointer">
            <a href="https://misland.readthedocs.io/en/latest/Service/Calculate_SDG.html#compute-sdg-15-3-1-sub-indicators"
...
export default {
  ...
  data() {
    return {
      ...
      veg_index: "NDVI",
      end_year: 2019,
      indicator: { label: "Performance", value: "performance", colors },
  ...
  methods: {
    ...
    async fetchYears() {
      this.years = await fetchComputationYears("Productivity Performance")
    },
    ...
    async submit() {
      try {
        let payload = {
          ...
          computation_type: "Productivity Performance",
          dashboard_context: 1,
        }
        //reset data
        this.summary_html = "";
        ...
        const uri = `/api/performance/`; // build the request url
        this.request_pending = true
        const results = await requestAnalysis({ uri, payload, caller: 'performance', indicator: this.indicator, hide_loading_progress: true });// request analysis
    ...
    processStatsSummary() {
      // summary for state
      this.summary_html = this.$t("summaries.performance", [
