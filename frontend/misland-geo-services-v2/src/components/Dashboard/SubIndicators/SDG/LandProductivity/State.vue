...
export default {
  ...
  data() {
    return {
      ...
      veg_index: "NDVI",
      end_year: 2019,
      indicator: { label: "State", value: "state", colors },
  ...
  methods: {
    ...
    async fetchYears() {
      ...
      fetchPrecomputationYears("Productivity State")
    },
    ...
    async submit() {
      try {
        let payload = {
          ...
          computation_type: "Productivity State",
          dashboard_context: 1,
        }
        //reset data
        this.summary_html = "";
    ...
    processStatsSummary() {
      // summary for state
      this.summary_html = this.$t("summaries.state", [
