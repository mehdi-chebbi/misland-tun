  ...
  <div class="chart-container q-my-lg">
    ...
    <div v-show="!request_pending">
      <div class="flex justify-between">
        <div class="indicator-chart-title">{{ getRenderedCustomGeometryData?.name }} Trajectory </div>
        <!---------------------------------------  chart actions ----------------------------------------->
        <div class="chart-actions flex">
          ...
          <!-- link to information -->
          <div flat class="q-ma-xs cursor-pointer">
            <a href="https://misland.readthedocs.io/en/latest/Service/Calculate_SDG.html#compute-sdg-15-3-1-sub-indicators" target="_blank"
            ...
            <div class="dashboard-form-menu" v-show="show_selection_form">
              ...
              <div class="dashboard-product-selections">
                ...
                <div class="" v-if="show_advance_selections">
                  ...
                  <q-select hide-dropdown-icon outlined dense v-model="veg_index" behavior="menu"
...
export default {
  ...
  data() {
    return {
      ...
      veg_index: "NDVI",
      end_year: 2019,
      indicator: { label: "Trajectory", value: "trajectory", colors },
  ...
  methods: {
    ...
    async fetchYears() {
      this.years = await fetchComputationYears("Productivity Trajectory")
    },
    ...
    async submit() {
      try {
        let payload = {
          ...
          computation_type: "Productivity Trajectory",
          dashboard_context: 1,
        }
        //reset data
        this.summary_html = "";
        ...
        const uri = `/api/trajectory/`; // build the request url
        this.request_pending = true
        const results = await requestAnalysis({ uri, payload, caller: 'trajectory', indicator: this.indicator, hide_loading_progress: true });// request analysis
    ...
    processStatsSummary() {
      // summary for state
      this.summary_html = this.$t("summaries.trajectory", [
