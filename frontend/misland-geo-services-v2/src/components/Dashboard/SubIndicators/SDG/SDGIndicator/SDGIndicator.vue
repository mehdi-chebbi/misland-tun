  ...
  <div class="chart-container q-my-lg">
    ...
    <div v-show="!request_pending">
      <div class="flex justify-between">
        <div class="indicator-chart-title">{{ getRenderedCustomGeometryData?.name }} state </div>
        <!---------------------------------------  chart actions ----------------------------------------->
            ...
            <div class="dashboard-form-menu" v-show="show_selection_form">
              ...
              <div class="dashboard-product-selections">
                ...
                <div class="" v-if="show_advance_selections">
                  ...
                  <q-select hide-dropdown-icon outlined dense v-model="eco_unit" behavior="menu" :options="ecoUnits">
                    <template v-slot:append>
                      <q-icon name="keyboard_arrow_down" color="grey-6" />
                    </template>
                  </q-select>
                  ...
                  <q-select hide-dropdown-icon outlined dense v-model="soc_ref" behavior="menu" :options="socRefs">
                    <template v-slot:append>
                      <q-icon name="keyboard_arrow_down" color="grey-6" />
                    </template>
                  </q-select>
...
export default {
  ...
  data() {
    return {
      ...
      end_year: 2015,
      veg_index: "NDVI",
      eco_unit: "2000", // eco units
      soc_ref: 2000, // selected soc ref
  ...
  methods: {
    ...
    async submit() {
      try {
        ...
        const results = await requestAnalysis({ uri, payload, caller: 'soc', indicator: this.indicator, hide_loading_progress: true });// request analysis
    ...
    processStatsSummary() {
      // summary for state
      this.summary_html = this.$t("summaries.land_degradation", [
