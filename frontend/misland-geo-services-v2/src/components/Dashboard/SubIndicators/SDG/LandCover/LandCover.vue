  ...
  <div class="chart-container q-my-lg">
    ...
    <div v-show="!request_pending">
      <div class="flex justify-between">
        <div class="indicator-chart-title">{{ getRenderedCustomGeometryData?.name }} Land cover </div>
        <!---------------------------------------  chart actions ----------------------------------------->
...
export default {
  ...
  data() {
    return {
      ...
      start_year: 2015,
      end_year: "",
      indicator: { label: "Land Cover", value: "landcover" },
  ...
  methods: {
    ...
    async submit() {
      try {
        ...
        const results = await requestAnalysis({ uri, payload, caller: 'landcover', indicator: this.indicator, hide_loading_progress: true });// request analysis
        this.chart_data = chartData.dataProcessor({ results, indicator: this.indicator }); // create chart data for change indicators
    ...
    processStatsSummary() {
  // Define the land cover categories with their corresponding values
  const categories = [
    { label: "Forest", val: 7 },
    { label: "Grassland", val: 6 },
    { label: "Wetland", val: 5 },
    { label: "Bareland", val: 4 },
    { label: "Artificial", val: 3 },
    { label: "Cropland", val: 2 },
    { label: "Water", val: 1 }
  ];

  // Generate a filtered and formatted summary dynamically
  const summaryParts = categories
    .map(({ label, val }) => {
      const percentage = getPercentage(getStatValue({ label, val }));

      // If percentage is "0%", exclude this category entirely
      if (percentage === "0%") return null;

      // Handle special case for "Artificial areas" with "(less than 1%)"
      if (label === "Artificial" && percentage === "(less than 1%)") {
        return `${percentage} ${label} areas`;
      }
  ...
  // Ensure the final summary includes only the valid categories
  const summaryString = summaryParts.join(", ");

  // Replace the `summary_html` strictly with the filtered content
  this.summary_html = `In ${this.start_year}, the Land Cover for ${
    getRenderedCustomGeometryData.value?.name
  } was composed of ${summaryString}.`;
}
,
    // download chart image
    downloadChartPng() {
