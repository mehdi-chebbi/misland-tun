...
#shapefiles_data_dir = '/home/nyaga/django-apps/ldms_site/ldms/data/shapefiles/'
# shapefiles_data_dir = '/home/sftdev/django-apps/ldms_site/ldms/data/shapefiles/'
# shapefiles_data_dir = '/home/sftdev/django-apps/ldms/ldms/data/shapefiles/'
...
kenya_imported = False
sudan_imported = False
rwanda_imported = False
uganda_imported = False	
eritrea_imported = False
somalia_imported = False
djibouti_imported = False
ethiopia_imported = False
south_sudan_imported = False
east_africa_imported = True
...
def import_regional_level_shape_files():
	vector_util.load_regional_admin_shapefile(shapefiles_data_dir + 'Regional/Regional_Layer_LaSWE.shp')
...
def import_level_zero_shape_files():
	...
	#east africa
	if not east_africa_imported:
		vector_util.load_admin_zero_shapefile(shapefiles_data_dir + 'EastAfrica/WIND AOI.shp')

def import_level_one_shape_files():
	# Kenya
	if not kenya_imported:
		vector_util.load_admin_one_shapefile(shapefiles_data_dir + 'Level1/KENYA_LEV_1.shp')
	#Sudan
	if not sudan_imported:
		vector_util.load_admin_one_shapefile(shapefiles_data_dir + 'Level1/SUDAN_LEV_1.shp')
	#Rwanda
	if not rwanda_imported:
		vector_util.load_admin_one_shapefile(shapefiles_data_dir + 'Level1/RWANDA_LEV_1.shp')
	#Uganda
	if not uganda_imported:
		vector_util.load_admin_one_shapefile(shapefiles_data_dir + 'Level1/UGANDA_LEV_1.shp')
	#Eritrea
	if not eritrea_imported:
		vector_util.load_admin_one_shapefile(shapefiles_data_dir + 'Level1/ERITREA_LEV_1.shp')
	#Somalia
	if not somalia_imported:
		vector_util.load_admin_one_shapefile(shapefiles_data_dir + 'Level1/SOMALIA_LEV_1.shp')
	# Djibouti
	if not djibouti_imported:
		vector_util.load_admin_one_shapefile(shapefiles_data_dir + 'Level1/DJIBOUTI_LEV_1.shp')
 	#Ethiopia
	if not ethiopia_imported:
		vector_util.load_admin_one_shapefile(shapefiles_data_dir + 'Level1/ETHIOPIA_LEV_1.shp')
	# South Sudan
	if not south_sudan_imported:
		vector_util.load_admin_one_shapefile(shapefiles_data_dir + 'Level1/SOUTHSUDAN_LEV_1.shp')
...
def clip_raster(level, adminid):
	from common_gis.utils.raster_util import (get_raster_values, reproject_raster, clip_raster_to_vector)
